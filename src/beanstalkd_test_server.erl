...
%%%-------------------------------------------------------------------
-module(beanstalkd_test_server).
-behaviour(gen_server).

%% API
-export([manual_start/0, start_link/0, start_listener/1, start_listener/2,
...
manual_start() ->
    application:start(beanstalkd_test_server).
...
stop_listener(Ref) ->
    beanstalkd_test_server_fsm:stop(get_pid(Ref)).

get_uri(Ref) ->
    Port = get_port(Ref),
    "beanstalk://127.0.0.1:" ++ integer_to_list(Port) ++ "/".
...
handle_call({start_listener, Ref, Port}, From, State=#state{monitors=Monitors}) ->
    {ok, Pid} = beanstalkd_test_server_fsm_sup:start_fsm(Ref, From, Port),
...
start_stop_test() ->
    {ok, Pid} = beanstalkd_test_server:start_listener(testbeanstalkd),
    Pid = beanstalkd_test_server:get_pid(testbeanstalkd),
    Port = beanstalkd_test_server:get_port(testbeanstalkd),
    URI = "beanstalk://127.0.0.1:" ++ integer_to_list(Port) ++ "/",
    URI = beanstalkd_test_server:get_uri(testbeanstalkd),
    ok = beanstalkd_test_server:stop_listener(testbeanstalkd).
